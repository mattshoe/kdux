...
- **Dependency Injection:** Each component (middleware, store, enhancer, reducer) in **Kdux** can be fully decoupled from 
  ...
  manually providing them at the time of invocation and keeps each component completely separated and unrelated to each 
  other until assembled in the store. Functional designs tend to rely on accessing dependencies from outer scopes,
  which results in monolithic files of spaghettified logic. Integrating dependencies directly into your components
  **_significantly_** reduces this pattern.
